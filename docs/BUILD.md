...
`make clean` removes disposable files under `build/`, plus packaging build
artifacts under `dist/` and `src/weftmark.egg-info/` produced by `make smoke`
(`uv build`). It deliberately preserves the committed rev0 figures and logo
derivatives; refresh those with the explicit `figures` and `logo` targets
when their canonical sources change.
