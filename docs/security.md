...
Adapter transports refuse cross-origin redirects, cap each response body, and
reject a plaintext base URL when a token is configured, so a hostile forge cannot
exfiltrate the credential, redirect the authenticated request to an internal host,
or exhaust local memory (WM-T15–WM-T17).
...
| WM-T11 | Oversized requests, output, or remote responses exhaust local resources. | HTTP control bodies and evidence runtime are bounded; provider calls use bounded pagination and read each forge response body under a fixed cap (WM-T17). | Raw command output is captured in memory before hashing and the ledger has no quota. Run untrusted commands in a resource-limited environment and monitor storage. |
...
| WM-T14 | A Change Set's declared scope is widened without a matching lock, letting a different claim concurrently acquire the same file/contract with no conflict detected. | `ClaimService.extend_scope()` is the only operator path to widen scope: it requires the caller to be the claim's owning agent/session, requires the claim to be currently active, and acquires a new `SemanticLock` for each added scope — checked against every other active claim exactly like `acquire()` — before the Change Set's declared scope widens. `ChangeSet.amend_scope()`/`WorkspaceService.amend_scope()` remain unprivileged domain/application primitives and are not exposed to operators directly. | Found and fixed during this capability's own independent review, before it shipped as an operator-facing path. Any future caller of the lower-level `WorkspaceService.amend_scope()` must go through `ClaimService.extend_scope()` for the lock to stay in step with the declared scope. |
| WM-T15 | A malicious, compromised, or MITM'd forge answers a read with an HTTP redirect to another origin, to exfiltrate the configured forge credential or pivot the authenticated request to an internal host (SSRF). | Each forge transport opens through an opener that refuses any redirect changing scheme, host, or port; a cross-origin redirect becomes an `unavailable` observation, so the `Authorization` header is never resent to another origin and the request cannot be bounced to an internal service. Same-origin redirects (e.g. trailing-slash normalization) still follow. | The configured base host itself is trusted. Point adapters only at forges you trust, and prefer network egress controls for defense in depth. |
| WM-T16 | A forge adapter is configured with a token but a plaintext `http://` base URL, sending the credential in the clear. | Each adapter refuses an `http://` api_base when a token is configured, failing construction rather than transmitting the token over plaintext. `http://` without a token, and `https://` with a token, are unchanged. | Transport security still depends on the operator's TLS configuration and certificate trust; a token remains only as protected as the forge endpoint and network path. |
| WM-T17 | A hostile or malfunctioning forge returns an unbounded HTTP response body (on a success or error status) to exhaust local memory. | Every forge transport reads each response body through a fixed cap (16 MiB per REST page; pagination is separately bounded), on both the success and HTTP-error paths; an oversized body becomes an `unavailable` observation rather than being buffered whole. | The cap bounds a single response, not total process memory across many concurrent observations. Run untrusted-forge observation under a resource-limited environment (see WM-T11). |
