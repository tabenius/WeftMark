...
Later reconnects require the same Git common directory and require the claimed
base to remain an ancestor, so legitimate agent commits are accepted while a
replaced, unrelated, or history-rewritten worktree is refused.
...
proof of process isolation. Provider processes inherit the invoking process's OS
identity and environment unless the operator applies a separate sandbox.

ACP and control-host messages are strict JSON and bounded to 1 MiB. Text-file
callbacks accept regular UTF-8 files and string content up to 4 MiB; line/limit
parameters are bounded and type checked. Unsupported protocol versions,
malformed capabilities, session IDs, response envelopes, and stop reasons fail
as transport errors rather than being normalized into valid state.
...
configuration is delivered to the host over its initial private stdin pipe. The
socket identity includes the exact provider name and argv, while the application
ledger stores only a digest of name, argv, and declared capabilities. A later CLI
invocation with the same name but different configuration is refused instead of
silently attaching to the existing worker.
